<div class="w-100" id="pagination-loading">
    <img src="{{ secure_asset('site_assets/img/loading.gif') }}" style="display: block;margin: 0 auto">
</div>
<footer class="d-none d-sm-block">
    <div class="container">
        <div class="row pb-3">
            <div class="col-md-3">
                All rights reserved for <a href="{{ secure_url('/') }}" class="shoptizer-color">shoptizer 2018 </a>.
            </div>
            <div class="col-md-6">
            ...
            <div class="col-md-3 ">
                <div class="social float-right">
                    <a href="{{ $base_info->apple_store}}"><i class="fab fa-apple"></i></a>
                    <a href="{{ $base_info->google_play }}"><i class="fab fa-android"></i></a>
                    <a href="{{ $base_info->facebook }}"><i class="fab fa-facebook-f"></i></a>
                    <a href="{{ $base_info->instagram }}"><i class="fab fa-instagram"></i></a>
                    <a href="{{ $base_info->youtube }}"><i class="fab fa-youtube"></i></a>
                </div>
            </div>
    ...
    <script src="{{ secure_asset('site_assets/js/jquery.js') }}"></script>
    <script src="{{ secure_asset('site_assets/js/popper.js') }}"></script>
    <script src="{{ secure_asset('site_assets/js/bootstrap.js') }}"></script>
    <script src="{{ secure_asset('site_assets/js/totop.js') }}"></script>
    <script src="{{ secure_asset('site_assets/js/all.js') }}"></script>
    <script src="{{ secure_asset('site_assets/slick/slick.js') }}" type="text/javascript" charset="utf-8"></script>
...
<script>
    $(document).on('click','.view-icon',function () {
        ...
        $.ajax({
            type:'get',
            url:'{{ secure_url('ajax-product') }}'+'/'+id,
            dataType: 'html',
            success:function (data) {
...
<script>
    $(document).on('click','.fa-heart',function () {
       ...
       if(!isGuest){
           ...
           window.location.href = "{{ secure_url('sign') }}";
       }
    });
</script>
<script>
    function printProducts(data,is_ad,get_void,is_animate) {
        ...
        for (i=0;i< data.length;i++){
            ...
            var image = '{{ secure_url('') }}'+'/'+data[i]["image"];

            var url = '{{ secure_url('product') }}'+'/'+data[i]["id"];
            if (is_ad) {
            ...
            var product = '<div class="col-xs-12 product'+animate+'" data-id="'+id+'">\n' +
                ...
                '                                    <span class="actions">\n' +
                '                                        <a '+quick_cart+'>\n' +
                '                                            <img src="{{ secure_asset("site_assets/img/cart.png") }}" class="product-cart">\n' +
                '                                        </a>\n' +
                '                                        <a>\n' +
                ...
                '                                            <img src="{{ secure_asset("site_assets/img/eye.png") }}">\n' +
                '                                    </span>\n' +discount+
